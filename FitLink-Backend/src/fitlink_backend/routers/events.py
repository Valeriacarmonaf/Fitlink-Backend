# src/fitlink_backend/routes/events.py
...
@router.get("")
def list_events(limit: int = Query(50, ge=1, le=200), estado: Optional[str] = None):
...
def create_event(payload: EventCreate):
    try:
        inicio_utc = datetime.combine(payload.fecha, payload.hora).replace(
            tzinfo=timezone.utc
        ).isoformat()
...
def confirmar_evento(event_id: int):
    try:
        ...
        supabase.table("eventos").update({"estado": "confirmado"}).eq("id", event_id).execute()
...
def cancelar_evento(event_id: int):
    try:
        ...
        supabase.table("eventos").update({"estado": "cancelado"}).eq("id", event_id).execute()
